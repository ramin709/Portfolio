...
const ProjectCard: FunctionComponent<ProjectCardProps> = ({ project, currentProject, setCurrentProject }) => {

    const { name, description, github_url, deploy_url, image_src, tags, category, id } = project;
    ...
    return (
        <>
            <div onClick={() => setCurrentProject(id)}>
...

            {
                ...
                <motion.div className={style.card} initial="initial" animate="animate" variants={outerAnimation}>
                    <div className={style.cardTop}>
                        <Image src={image_src} width="400px" height="300px" alt={name} className={style.imgProjectCard} loading="lazy" />
                    ...
                    <MdClose onClick={() => setCurrentProject(null)} className={style.icon} />
                </motion.div>
            }
        </>
    )
